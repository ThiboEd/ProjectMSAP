...
public:
    ...
    Color3f eval(const BSDFQueryRecord& bRec) const {
        ...
        Color3f fr = D*F*G/(4*Frame::cosTheta(bRec.wi)*Frame::cosTheta(bRec.wo));
    ...
    Color3f sample(BSDFQueryRecord& bRec, const Point2f& _sample) const {
        ...
        //Calculate the reflected direction wo
...
public:
    ...
    Color3f eval(const BSDFQueryRecord &bRec) const {
        ...
        Color3f f_diff = 28*m_kd->eval(bRec.uv)/(23*M_PI)*(1-(std::pow(((m_extIOR-m_intIOR)/(m_extIOR+m_intIOR)),2)))*(1-std::pow((1-0.5*Frame::cosTheta(bRec.wi)),5))*(1-std::pow((1-0.5*Frame::cosTheta(bRec.wi)),5));
        ...
        float G = Reflectance::G1( bRec.wi , wh, alpha)*Reflectance::G1( bRec.wo , wh, alpha);
        Color3f f_mf = D*F*G/(4*Frame::cosTheta(bRec.wi)*Frame::cosTheta(bRec.wo));
        
        Color3f fr = f_diff + f_mf;
    ...
    float pdf(const BSDFQueryRecord &bRec) const {
       ...
       is wrong, or when queried for illumination on the backside */
        ...
        float prob_mf = Reflectance::fresnel(Frame::cosTheta(bRec.wi), m_extIOR, m_intIOR);
        float prob_diff = 1 - prob_mf;

        return (prob_mf*pdf_mf) + (prob_diff*pdf_diff);
    }
