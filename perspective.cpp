...
public:
    PerspectiveCamera(const PropertyList &propList) {
        ...
        m_lensRadius = propList.getFloat("lensRadius", 0.0f);  // Default: pinhole camera
        m_focalDistance = propList.getFloat("focalDistance", 1.0f);  // Default: 1 unit
    ...
    Color3f sampleRay(Ray3f &ray,
        ...
        return Color3f(1.0f);
                ...
                // 2. Calculate the point in the focus plane 
        Point3f focusPoint = Point3f(0, 0, 0) + d * m_focalDistance;

        // 3. Sample a point in the len
        Point2f lensSample = m_lensRadius * Warp::squareToUniformDisk(apertureSample);
        Point3f lensPoint(lensSample.x(), lensSample.y(), 0.0f);

        // 4. Calculate the new ray
        ray.o = m_cameraToWorld * lensPoint;
        ray.d = m_cameraToWorld * (focusPoint - lensPoint).normalized();
